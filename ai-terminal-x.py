...
def configure_ai(api_key):
    ...
    try:
        ...
        sys.exit(1)
def translate_to_english(model, roman_urdu_input):
    """
    Uses Gemini AI to translate Roman Urdu (or any non-English input) to English.
    Returns translated English text or original input on failure.
    """
    prompt = f"""
    You are a translation assistant. Translate the following Roman Urdu (or any non-English text)
    into clear English, while preserving its technical meaning. If the input is already in English,
    return it unchanged. Do NOT include any extra text, explanation, or formatting.

    Input: {roman_urdu_input}
    """
    try:
        response = model.generate_content(prompt)
        translation = getattr(response, 'text', '').strip()
        if translation:
            print(f"{green}Translated to English:{reset} {translation}")
            return translation
        else:
            print(f"{yellow}Translation failed. Using original input.{reset}")
            return roman_urdu_input
    except Exception as e:
...
{cyan}========================================================================={reset}
{blue}      AI Assisted Linux Terminal Assistant{reset} {gold}|{reset} Revolution Begins      
{cyan}========================================================================={reset}
"""
print(banner)
...
while True:
    ...
    while True:
        ...
        try:
            ...
            print(f"\n{blue}Choose input language:{reset}")
            print(f" {gold}[1] Roman Urdu{reset}")
            print(f" {gold}[2] English{reset}")
            language_choice = input(f"{green}Enter choice (1 or 2): {reset}").strip()

            if language_choice == "1":
                raw_input = input(f"{cyan}Enter command in Roman Urdu: {reset}").strip()
                user_input = translate_to_english(ai_model, raw_input)
            elif language_choice == "2":
                user_input = input(f"{cyan}Enter command in English: {reset}").strip()
            else:
                print(f"{red}Invalid choice. Defaulting to English.{reset}")
                user_input = input(f"{cyan}Enter command in English: {reset}").strip()
